...
WITH customer_cleaning AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN email_raw IS NOT NULL AND email_raw <> '' THEN 1
            ELSE 0
        END AS email_count,

        -- Clean numeric fields (convert TEXT to DECIMAL)
        CASE 
            WHEN credit_limit_raw IS NOT NULL AND credit_limit_raw <> '' AND credit_limit_raw <> 'NULL'
            THEN CAST(credit_limit_raw AS DECIMAL(15,2))
            ELSE 0.00
        END AS credit_limit,

        CASE 
            WHEN write_off_limit_raw IS NOT NULL AND write_off_limit_raw <> '' AND write_off_limit_raw <> 'NULL'
            THEN CAST(write_off_limit_raw AS DECIMAL(15,2))
            ELSE 0.00
        END AS write_off_limit,

        -- Keep timezone info in dates
        CASE 
            WHEN created_datetime_raw IS NOT NULL AND created_datetime_raw <> ''
            THEN CAST(created_datetime_raw AS TIMESTAMP)
            ELSE NULL
        ...
        CASE 
            WHEN created_datetime_raw IS NOT NULL AND created_datetime_raw <> ''
            THEN DATE(CAST(created_datetime_raw AS TIMESTAMP))
            ELSE NULL
        END AS created_date,

        CASE 
            WHEN last_modified_datetime_raw IS NOT NULL AND last_modified_datetime_raw <> ''
            THEN DATE(CAST(last_modified_datetime_raw AS TIMESTAMP))
            ELSE NULL
