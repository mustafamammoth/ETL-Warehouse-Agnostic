-- models/silver/sales_orders.sql
-- Cleaned, deduplicated, and standardized sales orders data (ClickHouse compatible)
{{ config(materialized='table') }}

/*
Phases:
  base          : bring in raw + derive _version_ts
  dedup         : keep latest row per order (order_guid or order_number fallback)
  order_cleaning: type casting / normalization (original logic + preserved fields)
...
dedup AS (
    ...
    FROM (
        SELECT
            *,
            row_number() OVER (
                PARTITION BY coalesce(nullIf(order_guid,''), nullIf(order_number,''))
...
order_categorization AS (
    SELECT
        *,
        ...
        -- net amount currently same as order_total
