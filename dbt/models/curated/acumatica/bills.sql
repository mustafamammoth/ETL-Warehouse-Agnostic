-- Cleaned and standardized bills data (silver layer) - ClickHouse compatible
{{ config(materialized='table') }}

WITH bill_cleaning AS (
    SELECT 
        bill_guid,
        ...
        -- Clean description (ClickHouse compatible - simplified for now)
        ...
        multiIf(
            ...
            parseDateTime64BestEffort(bill_date_raw),
        ...
        multiIf(
            (bill_date_raw IS NOT NULL) AND (bill_date_raw != ''),
            toDate(parseDateTime64BestEffort(bill_date_raw)),
...
bill_business_logic AS (
    SELECT
        *,
        ...
        -- Days outstanding calculation (ClickHouse compatible)
        multiIf(
